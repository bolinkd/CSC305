...
void Renderer::Render()
{
    ...
    if (*mSceneSP)
    {
        ...
        for (uint32_t instanceID : mScene->Instances)
        {
            ...
            uint32_t parentID = mScene->Instances[instanceID].ParentID;
            Instance* parent = &mScene->Instances[parentID];
            const Mesh* mesh = &mScene->Meshes[instance->MeshID];
            const Transform* ChildTransform = &mScene->Transforms[instance->TransformID]; 
